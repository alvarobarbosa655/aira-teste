...
class EtapaMensalCronograma(BaseModel):
    mes_referencia: str = Field(
        ...,
        description=(
            "Período da etapa. Use mês individual do Mês 1 ao Mês 12 (ex: 'Mês 1', 'Mês 2'). "
            "Do Mês 13 ao 24, agrupe por trimestre (ex: 'Mês 13-15', 'Mês 16-18'). "
            "Acima de 24, agrupe por semestre (ex: 'Mês 25-30', 'Mês 31-36')."
        ),
    )
...
class PontoEvolucao(BaseModel):
    mes_numero: int = Field(..., description="Mês do marco (ex: 0, 3, 6, 9, 12, 15, 18, 21, 24, 30, 36)")
...
--- DIRETRIZES DO PLANO E CURVA DE RECUPERAÇÃO ---
...
4. Especifique o plantio: espaçamento técnico, densidade de mudas/ha, custo unitário e total de mudas, preparo do solo/coveamento, contenção de voçorocas e pelo menos 4 espécies nativas adequadas (com nome científico).
5. CRONOGRAMA — REGRA OBRIGATÓRIA DE GRANULARIDADE:
   - Do Mês 1 ao Mês 12: detalhe MÊS A MÊS, cada mês é uma entrada separada no 'cronograma_mensal'.
   - Do Mês 13 ao Mês 24: agrupe por TRIMESTRE (Mês 13-15, Mês 16-18, Mês 19-21, Mês 22-24).
   - Acima de 24 meses: agrupe por SEMESTRE (Mês 25-30, Mês 31-36).
   - NUNCA faça agrupamentos maiores que 6 meses.
   Para cada período, detalhe: ações práticas de campo, meta ecológica esperada, e insumos/ferramentas necessários.
6. Gere a 'projecao_recuperacao' com marcos nos meses: 0, 3, 6, 9, 12, 15, 18, 21, 24, 30 e 36, estimando:
   - Cobertura Vegetal (%)
...
def chamar_agente_gemini(api_key: str, dados_campo: dict, status_callback=None) -> DiagnosticoCompleto:
    ...
    for idx, modelo in enumerate(MODELOS_GEMINI_FALLBACK):
...
# ==========================================================================
# PROCESSAMENTO COM INDICADOR DE PROGRESSO
# ==========================================================================
if btn_processar:
...
if st.session_state.diagnostico:
    ...
    aba_bioma, aba_cronograma, aba_plantio, aba_orcamento = st.tabs(
        [
            ...
            "2. Cronograma & Curva de Recuperação",
    ...
    # -----------------------------------------------------------------
    # ABA 2: CRONOGRAMA DETALHADO & CURVA DE RECUPERAÇÃO
    # -----------------------------------------------------------------
    with aba_cronograma:
        st.markdown("##### Projeção de Recuperação Ecológica ao Longo do Tempo")
        st.caption("Evolução estimada dos indicadores ambientais (cobertura vegetal, controle de erosão e infiltração hídrica):")
        ...
        if d.projecao_recuperacao:
            ...
            st.line_chart(df_evolucao, height=300)

        st.divider()
        st.markdown("##### Planejamento Executivo de Intervenção")
        st.caption("Ações detalhadas organizadas por período do ciclo de restauração:")
    ...
    with aba_orcamento:
        ...
        with col_o2:
            ...
            cor_saldo = "#16A34A" if d.dentro_do_teto_16k else "#DC2626"
            st.markdown(
                f"""
                <div class="box-indicador">
                    <div class="rotulo-indicador">Teto Conservação Internacional (R$ 16.000,00)</div>
                    <div class="valor-indicador">{status_teto} <span style="font-size:0.9rem;font-weight:500;color:{cor_saldo};">(Saldo: R$ {saldo:,.2f})</span></div>
                </div>
                """,
                unsafe_allow_html=True,
            )
